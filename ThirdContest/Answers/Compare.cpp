...
//
// modified: default float tolerance abs/rel = 1e-6
...
void wrong_answer(const char *err, ...) {
    ...
    fprintf(judgemessage,
            "Wrong answer on line %d of output (corresponding to line %d in answer file)\n",
...
void compare_float(const std::string &judge, const std::string &team,
                   flt jval, flt tval,
                   flt float_abs_tol, flt float_rel_tol,
                   const std::string &extra_msg) {
    if (std::isfinite(tval) && std::isfinite(jval)) {
        flt absdiff = fabsl(tval - jval);
        // If expected is 0, relative error is meaningless: force it "infinite"
        flt reldiff = (jval == 0) ? std::numeric_limits<flt>::infinity()
                                  : fabsl((tval - jval) / jval);
        ...
        if (float_abs_tol >= 0 && float_rel_tol >= 0) {
            if (absdiff > float_abs_tol && reldiff > float_rel_tol) {
                wrong_answer("Too large difference.\n Judge: %s\n Team: %s\n "
                             ...
                             absdiff, float_abs_tol, reldiff, float_rel_tol,
                             extra_msg.c_str());
            }
            ...
            if (absdiff > float_abs_tol) {
                wrong_answer("Too large difference.\n Judge: %s\n Team: %s\n "
                             ...
                             judge.c_str(), team.c_str(),
                             absdiff, float_abs_tol, extra_msg.c_str());
            }
            ...
            if (reldiff > float_rel_tol) {
                wrong_answer("Too large difference.\n Judge: %s\n Team: %s\n "
                             "Relative difference: %Lg (tolerance: %Lg)%s",
                             judge.c_str(), team.c_str(),
                             reldiff, float_rel_tol, extra_msg.c_str());
            }
        }
        ...
        if (std::signbit(jval) != std::signbit(tval)) {
            wrong_answer("Expected float %s, got: %s%s",
...
int main(int argc, char **argv) {
    ...
    // Defaults: enable float check with 1e-6 abs/rel tolerance
    bool case_sensitive = false;
    bool space_change_sensitive = false;
    bool use_floats = true;
    flt float_abs_tol = 1e-6L;
    flt float_rel_tol = 1e-6L;

    // Allow overrides by flags
    ...
    use_floats = use_floats || float_abs_tol >= 0 || float_rel_tol >= 0;

    judgeans_pos = stdin_pos = 0;
